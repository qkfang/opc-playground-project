...
@description('Enable the live Foundry agent path. When false the app uses its deterministic offline FinOps engine.')
param foundryEnabled bool = true

@description('Model deployment name for the FinOps agent (a smaller model is recommended for Fabric-tool orchestration)')
...
@description('Blob container for chat transcripts')
param containerName string = 'transcripts'

@description('Microsoft Fabric data agent Foundry connection id (optional; enables the Fabric data-agent tool). Format: /subscriptions/.../accounts/<foundry>/projects/<project>/connections/<name>')
param fabricConnectionId string = ''

@description('Optional Fabric workspace GUID (diagnostics/display only)')
param fabricWorkspaceId string = ''

@description('Optional Fabric data agent artifact GUID (diagnostics/display only)')
